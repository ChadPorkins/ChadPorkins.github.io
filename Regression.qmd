...
After obtaining the correlation results, a standard linear model will be conducted to further analyze the relationship between the Lst data and the Temperature variables. To obtain the most suitable model, the `Stepwise Algorithm` will be used.
...
```{r}
library(ggplot2)
ggplot(summer_data, aes(x = fitted(model), y = residuals(model))) +
  geom_point() +
